...
## MCP tools: gateway with curated surface

MCP servers (signoz, argocd) sit behind a local mcp-proxy gateway, NOT loaded
raw. Only the most-used tools are directly mounted (signoz: 8, argocd: 3, plus
`proxy/*` admin). The full catalog (43 signoz + 16 argocd tools) exists but is
hidden to keep context lean.

- Discover more: call `proxy/search_tools` with a natural-language query
  (e.g. "dashboard alerts"). It BM25-searches the FULL catalog and returns
  names + descriptions.
- Hidden tools CANNOT be invoked directly ("Tool not available" is expected).
  If the task needs one, tell the user to add it to `expose_tools` in
  `~/Projects/dotfiles/configs/mcp-proxy/proxy.toml` — it hot-reloads, no
  restart needed.
...
## Shell output: rtk (pi only)

pi loads the `pi-rtk` package — a CLI proxy that filters and summarizes command
output before it reaches context, saving tokens. Prefer running supported
commands through it: `rtk git …`, `rtk grep …`, `rtk test`, `rtk diff`,
`rtk log …`, `rtk docker …`, `rtk kubectl …`, etc.

Caveat: rtk output filters can mangle raw file contents — when byte-exact
output matters, read the file directly instead of `rtk cat`.
